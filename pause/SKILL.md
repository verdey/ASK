...
When writing the resume brief, read [`../ask/_src/surface-doctrine.md`](../ask/_src/surface-doctrine.md) — the `.test` surface map — to close the brief with the right entry point. The "Next immediate action" line should end with `http://alpha.test` as the live codebase entry unless a more specific surface applies.
